...
Column {
  ...
  Column {
    ...
    Item {
      ...
      MouseArea {
        anchors.fill: parent
        onClicked: {
          nativeUtils.openUrl(fileUtils.getDataStorageUrl(getInternalStorageLocation()))
        }
      }
    }
  ...
  function fillListWithFiles() {
    ...
    stringList = fileUtils.listFiles(fileUtils.getDataStorageUrl(getInternalStorageLocation()), ["*.json"])
    files = []
    for(i = 0; i < stringList.length; i++){
      files[i] = stringList[i]
    }
